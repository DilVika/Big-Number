...
std::string QINT::toDec()
{
	...
	std::string temp = std::to_string(0);
	for (int i = bit / 2; i < bit; ++i)
	{
		if ((high & 1))
		{
			redict = pow(2.0, (long double)i);
			temp = addByString(temp, convertDoubToInt(std::to_string(redict)));
		}
		high = high >> 1;
	}

	result = addByString(temp, std::to_string(low));
